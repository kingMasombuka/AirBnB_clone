...
def parse(arg):
    curl_brak = re.search(r"\{(.*?)\}", arg)
    brak = re.search(r"\[(.*?)\]", arg)
    if curl_brak is None:
        if brak is None:
            return [i.strip(",") for i in split(arg)]
        else:
            lexer = split(arg[:brak.span()[0]])
            retl = [i.strip(",") for i in lexer]
            retl.append(brak.group())
            return retl
    else:
        lexer = split(arg[:curl_brak.span()[0]])
        retl = [i.strip(",") for i in lexer]
        retl.append(curl_brak.group())
        return retl
...
class HBNBCommand(cmd.Cmd):
    ...
    def default(self, arg):
        ...
        if match is not None:
            ...
            if match is not None:
                ...
                if command[0] in argsd.keys():
                    call = "{} {}".format(arg_1[0], command[1])
                    return argsd[command[0]](call)
    ...
    def do_destroy(self, arg):
        ...
        if len(arg_1) == 0:
            ...
            print("** instance id missing **")
        elif "{}.{}".format(arg_1[0], arg_1[1]) not in objdict.keys():
            print("** no instance found **")
        else:
    ...
    def do_update(self, arg):
       ...
       <class>.update(<id>, <dictionary>)
        ...
        if len(arg_1) == 1:
            print("** instance id missing **")
            return False
        if "{}.{}".format(arg_1[0], arg_1[1]) not in objdict.keys():
            print("** no instance found **")
            return False
        if len(arg_1) == 2:
